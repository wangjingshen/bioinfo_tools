...
argv <- add_argument(argv,"--cluster_col",help = "colnames of the column in meta data which storing the cluster information",default = "cluster")
...
argv <- add_argument(argv,"--tsne",help = "whether do tsne reduction",default = "yes")
...
print(argv$test)
print(argv$test_test)
cluster <- argv$cluster_col
newcol <- argv$newcol
tsne <- argv$tsne
...
attributes(class(PRO_v3@reductions$pca@jackstraw)) <- list(package = "Seurat")
print("pca")


PRO_v3@reductions$umap <- PRO_v4@reductions$umap
attributes(attributes(PRO_v3@reductions$umap)$class) <- list(package = "Seurat")
attributes(class(PRO_v3@reductions$umap@jackstraw)) <- list(package = "Seurat")
print("umap")
if (!'tsne' %in% attributes(PRO_v4@reductions)$names) {tsne = 'no'}
if(tsne == "yes"){
PRO_v3@reductions$tsne <- PRO_v4@reductions$tsne
attributes(attributes(PRO_v3@reductions$tsne)$class) <- list(package = "Seurat")
attributes(class(PRO_v3@reductions$tsne@jackstraw)) <- list(package = "Seurat")
print("tsne")
}
if(newcol == "yes"){
