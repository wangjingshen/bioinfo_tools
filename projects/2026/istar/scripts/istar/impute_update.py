...
def get_patches_flat(img, locs, mask):
    ...
    for s in locs:
        ...
        ## fix null patch
        if patch.size == 0:
            if null_patch_count == 0:
                logger.info(f'a null patch example:')
                logger.info(f'patch.shape {patch.shape}')
                logger.info(f'mask.shape {mask.shape}')
            null_patch_count += 1
            patch = np.zeros((*mask.shape, patch.shape[2]), dtype=img.dtype)
    ...
    logger.info(f'find {null_patch_count} null patchs.')
