...
class Virus_gene_consensus:
    ...
    def get_virus_barcode_umi(self):
        count_detail = pd.read_csv(glob.glob(f"{self.fj_path}/08.count_capture_virus_mtx/*count_detail.txt")[0], sep="\t")
        ...
        count_detail_virus = count_detail.loc[ count_detail.Barcode.isin(virus_barcode), ].copy()
...
def dumb_consensus(read_list, threshold=0.5, min_consensus_read=1, ambiguous='N'):
    '''
