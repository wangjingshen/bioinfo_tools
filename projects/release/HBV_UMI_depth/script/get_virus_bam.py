...
class Bam_virus:
    def __init__(self, args):
        ...
        virus_tsne = pd.read_csv(virus_tsne_path, sep="\t", index_col=0).fillna(0)
        virus_barcode = list(virus_tsne['barcode'][virus_tsne.UMI!=0])
        virus_read_count = pd.read_csv(virus_read_count_path, sep="\t")
        virus_read_count = virus_read_count[(virus_read_count.read_count >= int(args.otsu_min_support_read)) & (virus_read_count.barcode.isin(virus_barcode)) ]
        self.virus_barcode_umi = list(virus_read_count.barcode + "_" + virus_read_count.UMI)
