...
class Probe_split:
    def __init__(self, args):
        ...
        self.otsu_min_support_read = args.otsu_min_support_read
        self.virus_barcode_umi = ''   # set by get_virus_barcode_umi
    ...
    def get_virus_barcode_umi(self):
        virus_tsne = pd.read_csv(glob.glob(f"{self.fj_path}/*analysis_capture_virus*/*virus_tsne.tsv")[0], sep="\t", index_col=0)
        virus_tsne = virus_tsne.fillna(0)   # fillna 0
        virus_barcode = list(virus_tsne['barcode'][virus_tsne.UMI!=0])
        otsu_min_support_read = int(self.otsu_min_support_read)
        virus_read_count = pd.read_csv(glob.glob(f"{self.fj_path}/*count_capture_virus*/*virus_read_count.tsv")[0], sep="\t")
        virus_read_count = virus_read_count[(virus_read_count.read_count >= otsu_min_support_read) & (virus_read_count.barcode.isin(virus_barcode)) ]
        self.virus_barcode_umi = list(virus_read_count.barcode + "_" + virus_read_count.UMI)   # report viurs barcode UMI 
    ...
    def split_bam(self):
        ...
        def probe_bam_out(probe_list, out_bam_name):
            ...
            ref_record_df_filter = ref_record_df[(ref_record_df.barcode_umi.isin(self.virus_barcode_umi)) & (ref_record_df.qname.isin(probe_list))]
    ...
    def run_consensus(self):
        def consensus(sample,fq):
            cmd1 = f"/SGRNJ/Public/Software/conda_env/celescope1.5.1b0/bin/celescope capture_virus consensus \
                --outdir outdir/3.consensus/{sample}/ \
                --sample {sample} \
                --assay capture_virus \
                --thread 4 \
                --threshold 0.5 \
                ...
                --fq {fq}"
            subprocess.check_call(cmd1, shell = True)
        consensus("S", "outdir/2.fastq/S.fq")
        consensus("X", "outdir/2.fastq/X.fq")
        consensus("pgRNA", "outdir/2.fastq/pgRNA.fq")
        consensus("rcDNA", "outdir/2.fastq/rcDNA.fq")
        consensus("cccDNA", "outdir/2.fastq/cccDNA.fq")
...
def main():
    ...
    parsers.add_argument('--otsu_min_support_read', help='min support read', required=True)
