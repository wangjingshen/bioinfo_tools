...
def istar2spots(istar_lables, spots_pos, outdir, foreground_method, k, distance_thresh, clip):
    ...
    if(foreground_method == "in_tissue"):
    ...
    # plt imshow
    plt.figure(figsize=(9, 8))
    plt.imshow(istar_cluster, cmap="tab10", alpha=0.2, origin="upper")  # show istar raw plot 
    plt.scatter(valid["x"], valid["y"], c=valid["cluster"], cmap="tab10", s=12, edgecolor="black", lw=0.2)
    plt.colorbar(shrink=0.7)
    plt.axis("off")
    plt.title("istar", fontsize=13)
...
def main():
    ...
    istar2spots(args.istar_labels, args.space_pos, args.outdir, args.foreground_method, args.k, args.distance_thresh, args.clip)
