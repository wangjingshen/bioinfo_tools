...
def plot_differential_scores(plot_data, group1, group2, title, plot_type, outdir, name):
    ...
    plt.savefig(f'{outdir}/compass_plot_{name}.pdf', dpi=200)
    ...
    if(plot_type == "anno"):
        for r in plot_data.index:
            ...
            if(plot_data.loc[r, 'cohens_d'] <= 0):
                ...
                offset = (10,-20)
                axs.annotate(r + plot_data.loc[r, 'reaction_name'], (x,y), xytext = offset, textcoords='offset pixels', arrowprops={'arrowstyle': "-"})
            plt.savefig(f'{outdir}/compass_plot_{name}_anno.pdf', dpi=200)
...
def run(reaction_penalties, metadata, group_metadata, subsystem, plot_type, outdir, name):
...
def main():
    ...
    run(args.reaction_penalties, args.metadata, args.group_metadata, args.subsystem, args.plot_type, args.outdir, args.name) 
