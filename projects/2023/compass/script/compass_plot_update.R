...
argv <- add_argument(argv, "--name", help = "output name, just like subsystem, special symbols need to be removed")
...
ggsave(paste0(argv$outdir, "/compass_plot_", argv$name, "update.pdf"), plot=p1, height=5, width=6)
ggsave(paste0(argv$outdir, "/compass_plot_", argv$name, "update.png"), plot=p1, height=5, width=6)

p2 <- plot_data %>%
    ggplot(aes(x = cohens_d, y = -log10(adjusted_pval))) +
    geom_text_repel(aes(label = reaction_name), size = argv$annotation_text_size) +
    geom_point() +
    xlim(c(-max(abs(plot_data$cohens_d)), max(abs(plot_data$cohens_d)))) +
    xlab(paste0("Cohen's d (", argv$xlab_name, ")"))+
    ylab('-log10(adjusted_pval)') +
    ...
    geom_hline(yintercept = -log10(0.05), color = "red", linetype = "dashed") +
    theme_classic() + theme(plot.title = element_text(hjust = 0.5))
ggsave(paste0(argv$outdir, "/compass_plot_", argv$name, "anno_update.pdf"), plot=p2, height=8, width=10)
ggsave(paste0(argv$outdir, "/compass_plot_", argv$name, "anno_update.png"), plot=p2, height=8, width=10)
