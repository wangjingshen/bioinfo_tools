...
def read_cell_names1(pathseq_bam_file, write_bac):
    ...
    for each_line in seqbam:
        ...
        if each_line.has_tag('YP'):
            ...
            #022422:use AS tag instead of mapping_quality  ## Alignment score generated by aligner
    ...
    for each_line in pathseq_report:
        ...
        if '|' in each_line_list[1]:
            ...
            for n in range(len(name_string_list)):
                pointer = -n-1           ## -1,-2,-3
                if not '_' in name_string_list[pointer]:  ## select genus
...
def generate_barcode_UMI_dict(out_readname_cell_path):#,pathseq_report_csv,out_genus_file,sample_ident):
    '''
    not match rna bc
    {'ATCGTTGGC_CACGCAATA_ACATTGCTG+CCCGTTACTATC': {'id_string': ['1292020','37915'],
                                                    'mapping_quality': 148,
                                                    'genus_string': 'Dietzia'},  
     'CACATTCAC_GTAGTTCCT_CAGACACTC+GGAACCGATTGT': {'id_string': ['1292020','37915'],
                                                    'mapping_quality': 142,
                                                    'genus_string': 'Dietzia'},
    ...
    }
    ...
    for each_line in cell_path_file:
        ...
        if not barcode_UMI in barcode_UMI_dict:
            ...
            barcode_UMI_dict[barcode_UMI]["genus_string"] = genus_list
            '''
        elif int(mapping_quality) > barcode_UMI_dict[barcode_UMI]["mapping_quality"]:  ## use max mapping_quality; may delete some tax_id ??
            barcode_UMI_dict[barcode_UMI]["id_string"] = id_string_list
            barcode_UMI_dict[barcode_UMI]["mapping_quality"] = int(mapping_quality) 
            barcode_UMI_dict[barcode_UMI]["genus_string"] = genus_string
...
def output_cells_genus_list(barcode_UMI_dict,dict_for_genus):
    '''
     'CGGTGATTG_TGACTTCCG_AGGCAGAAC': {'genus': ['Dietzia','Dietzia','Dietzia','Dietzia'],
                                       'barcode_UMI': {'CGGTGATTG_TGACTTCCG_AGGCAGAAC+GTTCACCTGAAT': 'Dietzia',
                                                       'CGGTGATTG_TGACTTCCG_AGGCAGAAC+GTTTACCTGAAT': 'Dietzia',
                                                       'CGGTGATTG_TGACTTCCG_AGGCAGAAC+CTTCACCTGAAT': 'Dietzia',
                                                       'CGGTGATTG_TGACTTCCG_AGGCAGAAC+GTACAGCTGAAT': 'Dietzia'},
                                       'pathogen_count': {'Dietzia': 4}},
     'CAACATCGG_TGAGGCCTT_GCGTCTAGT': {'genus': ['Rhodococcus'],
                                       'barcode_UMI': {'CAACATCGG_TGAGGCCTT_GCGTCTAGT+ACATAGTAGAGG': 'Rhodococcus'},
                                       'pathogen_count': {'Rhodococcus': 1}},
    ...
...
if __name__ == "__main__":
    ...
    print("UMI matrix: sample id: " + sample_ident)
    print("---")
