...
argv <- add_argument(argv, "--df_genus_path", help = "df genus path")
...
rds <- argv$rds
df_genus_path <- argv$df_genus_path
rna_spname <- unlist(strsplit(argv$rna_spname, split = ","))
spname <- unlist(strsplit(argv$spname, split = ","))
df_genus <- paste0(df_genus_path, "/", spname, "/", spname, "_genus_umi.tsv")
...
plot_total_barplot <- ifelse(is.na(argv$plot_total_barplot), "F", argv$plot_total_barplot)
...
write.table(df_sample_cluster, str_glue("{outdir}/cluster/sample_cluster.tsv"), quote=F, sep="\t", col.names=F)
...
df_genus <- lapply(1:length(df_genus), function(x){
    ...
    row.names(df) <- paste0(rna_spname[x], "_", row.names(df))  #
    return(df)
})
...
for (i in 1:length(rna_spname)){
    ...
    #df_genus_subset <- df_genus[ grep(paste0(rna_spname_subset, "_"), row.names(df_genus)),]  # get subset df_genus  # please check
    df_genus_subset <- df_genus[ colnames(data_seurat_subset),]
...
if(plot_total_barplot == "T"){
    ...
    plot_data_subset <- plot_data[ plot_data$umi>0, ]
    ...
    plot_data_subset <- plot_data_subset %>% 
        group_by(spname, genus) %>%
        summarise(sum_umi = sum(umi),
                  mean_umi = mean(umi))

    # genus order mean umi, set levels
    plot_data_subset$genus <- factor(plot_data_subset$genus, 
        levels = c(setdiff(unique(plot_data_subset$genus[order(plot_data_subset$mean_umi, decreasing=T)]), "others"), "others"))

    # df
    write.table(plot_data_subset, file = str_glue('{outdir}/cluster_genus_mean_umi/total_genus_mean_umi.tsv'), sep="\t", quote=F, row.names = F )
    # plot
    ggplot(plot_data_subset, aes(spname, mean_umi, fill=genus))+
        geom_bar(color="black", stat="identity", width = 0.2)+
        ggtitle("total")+
        ...
        theme(legend.title = element_blank(), 
              ...
              axis.title.x = element_blank(),
              axis.text.x = element_text(hjust = 1, vjust = 1, angle = 45))
    ggsave(str_glue('{outdir}/cluster_genus_mean_umi/total_genus_mean_umi_barplot.png'), width = barplot_width, height = 6)
    ...
    # genus order mean umi, set levels
    plot_data_subset$genus <- factor(plot_data_subset$genus, 
        levels = c(setdiff(unique(plot_data_subset$genus[order(plot_data_subset$sum_umi, decreasing=T)]), "others"), "others"))

    # df
    write.table(plot_data_subset, file = str_glue('{outdir}/cluster_genus_mean_umi/total_genus_umi.tsv'), sep="\t", quote=F, row.names = F )
   
    ggplot(plot_data_subset, aes(spname, sum_umi, fill=genus))+
        geom_bar(color="black", position = position_fill(), stat="identity", width = 0.2)+
        #geom_col(color="black", position = "stack", width = 0.2)+
        ggtitle("total")+
        scale_fill_manual(values = color_protocol)+
        ...
        theme(legend.title = element_blank(), 
              ...
              axis.title.x = element_blank(),
              axis.text.x = element_text(hjust = 1, vjust = 1, angle = 45))
    ggsave(str_glue('{outdir}/cluster_genus_mean_umi/total_genus_umi_barplot.png'), width = barplot_width, height = 6)
}
