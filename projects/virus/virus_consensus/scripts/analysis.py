...
class Virus_consensus:
    ...
    def get_virus_barcode_umi(self):
        '''
        filter_read_count: 05
        virus_tsne: 05
        '''
        filter_read_count = pd.read_csv(glob.glob(f"{self.fj_path}/*count_capture_virus/*filter_read_count.tsv")[0], sep="\t")
        ...
        filter_read_count_virus = filter_read_count.loc[ filter_read_count.barcode.isin(virus_barcode), ].copy()    # filter negative barcodes  # copy for deal with "A value is trying to be set on a copy of a slice from a DataFrame"
        filter_read_count_virus['barcode_umi'] = filter_read_count_virus.barcode + "_" + filter_read_count_virus.UMI
        # split gene
        virus_barcode_umi_list =  list(filter_read_count_virus.loc[:, 'barcode_umi'])
        ...
        probe_bam_out(virus_barcode_umi_list, f'{self.outdir}/1.bam/virus.bam')
    ...
    def run_consensus(self):       
        ...
        def consensus(gene):
            ...
            if(os.path.getsize(f"{self.outdir}/2.fasta/{gene}.fa")==0):
                Virus_consensus.run_consensus.logger.warning(f"{self.outdir} - {gene}.fa is null, skiping")
                out_h.close()
                return
            if(os.path.getsize(f"{self.outdir}/2.fasta/{gene}.fa")!=0):
                with pysam.FastxFile(f"{self.outdir}/2.fasta/{gene}.fa") as fh:
                    for (barcode, umi), g in groupby(fh, key=keyfunc):
                        ...
                        if n_umi % 1000 == 0:
                            Virus_consensus.run_consensus.logger.info(f'{self.outdir} - {gene}: {n_umi} UMI done.')
                    out_h.close()
                    Virus_consensus.run_consensus.logger.info(f'{self.outdir} - {gene} done.')
    ...
    def consensus_summary(self):
        def get_consensus_summary(gene):
            ...
            if(os.path.getsize(f"{self.outdir}/3.consensus/{gene}_consensus.fa")==0):
                Virus_consensus.consensus_summary.logger.warning(f"{self.outdir} - {gene}_consensus.fa is null, skiping")
                out_h.close()
                return
            if(os.path.getsize(f"{self.outdir}/3.consensus/{gene}_consensus.fa")!=0):
                ...
                Virus_consensus.consensus_summary.logger.info(f'{self.outdir} - {gene} consensus summary done.')
                        
        get_consensus_summary("virus")
